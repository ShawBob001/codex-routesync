...
Beim Öffnen der Aktivitätsleistenansicht **Codex SwitchBridge** stehen gespeicherte Konten und API-Anbieter auf derselben Ebene in einer flachen Liste **Accounts & API Routes**. Gleichzeitig wird das Dashboard automatisch geöffnet oder in den Vordergrund gebracht. In der Routenliste verwaltest du Konten und APIs. Das breite Dashboard zeigt Kontingente, Rücksetzzeiten, den automatischen Wechsel und den lokalen Token-Verlauf.

![Codex SwitchBridge-Dashboard auf Englisch im dunklen Design](./assets/screenshots/dashboard-en-dark.png)

Dasselbe Dashboard kann sofort auf vereinfachtes Chinesisch umgestellt werden:

![Codex SwitchBridge-Dashboard auf vereinfachtem Chinesisch im hellen Design](./assets/screenshots/dashboard-zh-light.png)
...
```text
Codex-Kontomodus  <->  Codex SwitchBridge  <->  Responses-API-Anbietermodus
                               |
                    gemeinsamer Verlauf unter CODEX_HOME
```

| Funktion | Verhalten von SwitchBridge |
| --- | --- |
...
Installiere die Erweiterung aus dem [Visual Studio Marketplace](https://marketplace.visualstudio.com/items?itemName=baoshichao001-dev.codex-switchbridge). Du kannst auch die Erweiterungsansicht in VS Code öffnen und nach `Codex SwitchBridge` oder `@id:baoshichao001-dev.codex-switchbridge` suchen.

Für eine Offline-Installation lädst du die neueste `.vsix`-Datei von den [GitHub-Releases](https://github.com/baoshichao001-dev/codex-switchbridge/releases) herunter und führst anschließend **Extensions: Install from VSIX...** aus. Alternativ kannst du den folgenden Befehl im Terminal verwenden. Ersetze VERSION durch die Version im Namen der heruntergeladenen Datei.
...
Verwende in VS Code **Switch Account** oder **Switch API Provider**. SwitchBridge speichert die aktuelle Auswahl, aktualisiert `auth.json` und `config.toml` und lädt danach die Konto- und Anbieteransichten neu.
...
SwitchBridge verhindert diese Trennung bei neuen Threads:

1. Der Kontomodus verwendet den integrierten Codex-Anbieter `openai`.
2. Ein Responses-kompatibler API-Anbieter behält dieselbe Verlaufsidentität bei, während SwitchBridge dessen API-Schlüssel und Basis-URL anwendet.
3. Beim Zurückwechseln werden die Anmeldedaten des Kontos und die ursprüngliche OpenAI-Route wiederhergestellt.

Beide Modi lesen deshalb denselben lokalen Gesprächsverlauf im selben `CODEX_HOME`. SwitchBridge synchronisiert die Route, die zur Indizierung des Verlaufs verwendet wird. Gesprächsinhalte werden nach einem Wechsel nicht kopiert.
...
2. Führe **Codex SwitchBridge: Repair Shared Conversation History** aus.
...
Einige Codex-Werkzeuge speichern die Authentifizierung beim Start im Cache. SwitchBridge kann einen anderen Erweiterungsprozess nicht dazu zwingen, diesen Cache zu verwerfen. Daher kann nach einem erfolgreichen Dateiwechsel weiterhin ein Neuladen des VS Code-Fensters erforderlich sein. Standardmäßig bleibt diese Empfehlung in der Statusleiste, statt wiederholt Pop-up-Hinweise anzuzeigen.

Führe **Codex Account Switch** und Codex SwitchBridge nicht gleichzeitig aus. Beide Erweiterungen schreiben in dieselben lokalen Codex-Dateien.
...
Codex SwitchBridge ist ein unabhängiges Open-Source-Projekt, das von [jqknono/codex-account-switch](https://github.com/jqknono/codex-account-switch) abgeleitet wurde. `baoshichao001-dev` hat es umfassend verändert.

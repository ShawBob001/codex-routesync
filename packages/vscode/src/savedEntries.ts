...
import { providerSwitchOptions } from "./sharedHistory";

export type StorageSource = "local" | "cloud";
export type SaveTarget = StorageSource;
const LOG_PREFIX = "[codex-switchbridge:vscode:savedEntries]";
...
const SYNCED_CLOUD_MIGRATION_KEY = "codex-switchbridge.syncedCloudStateMigration.v1";
...
const CURRENT_SELECTION_KEY = "codex-switchbridge.currentSavedSelection";
...
function getConfiguration() {
  return vscode.workspace.getConfiguration("codex-switchbridge");
}
...
function selectCurrentAccount(accounts: SavedAccountInfo[]): SavedAccountInfo[] {
  ...
  if (!currentAuth || getEffectiveActiveProvider()) {
    return accounts;
  }
...
export async function useSavedAccountEntry(
  ...
  activateAccountAuth(markCloudAuthForCurrentUse(account.auth, { updatedAt: account.syncUpdatedAt }), {
    source: selectionSwitchLabel(currentSelection),
    target: `account:cloud:${account.name}`,
  });
  await setMarker({
    kind: "account",
    name: account.name,
...
async function refreshCloudSavedAccountEntry(
  ...
  try {
    ...
    if (currentAuthWriteGuard && writeCurrentAuthIfUnchanged(currentAuthWriteGuard, auth)) {
...
export async function switchToSavedProviderEntry(
  ...
  try {
    activateProviderProfile(
      provider.profile,
      providerSwitchOptions(selectionSwitchLabel(currentSelection), `provider:cloud:${provider.name}`)
    );
  } catch (error) {
...
async function removeLocalProviderFile(name: string): Promise<void> {
  ...
  const core = await import("@codex-switchbridge/core");

...
import { isRefreshTokenExpiringWithin, isReloginRequiredRefreshError, isTokenExpired, isTokenExpiringWithin } from "@codex-switchbridge/core";
...
export class RefreshCoordinator implements vscode.Disposable {
  ...
  private restartAutoRefreshTimer(): void {
    ...
    const intervalSec = vscode.workspace.getConfiguration("codex-switchbridge").get<number>("quotaRefreshInterval", 30);
  ...
  private isAutoSwitchEnabled(): boolean {
    return vscode.workspace.getConfiguration("codex-switchbridge").get<boolean>("autoSwitchOnZeroQuota", false);
  }

  private isTokenAutoUpdateEnabled(): boolean {
    return vscode.workspace.getConfiguration("codex-switchbridge").get<boolean>("tokenAutoUpdate", true);
  }
  ...
  private async flushQuotaRefresh(): Promise<void> {
    ...
    try {
      ...
      void vscode.commands.executeCommand("codex-switchbridge.maybeAutoSwitchExhaustedAccount", {

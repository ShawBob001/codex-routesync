...
function createTempCodexHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "codex-switchbridge-"));
}
...
test("switchMode shared option routes a saved provider through openai", async (t) => {
  const codexHome = createTempCodexHome();
  process.env.CODEX_HOME = codexHome;
  ...
  const result = core.switchMode("cliproxyapi", {
    ...
    source: "account:work",
    target: "provider:cliproxyapi",
  });

  assert.equal(result.success, true);
  const config = fs.readFileSync(path.join(codexHome, "config.toml"), "utf8");
  assert.doesNotMatch(config, /^model_provider\s*=/m);
  assert.match(config, /^openai_base_url\s*=/m);
  assert.equal(core.getSharedHistoryRouteState().activeProvider, "cliproxyapi");
  assert.deepEqual(core.getCurrentSelection(), { kind: "provider", name: "cliproxyapi" });

  const deleteResult = core.deleteProviderProfile("cliproxyapi");
  assert.equal(deleteResult.success, false);
  assert.match(deleteResult.message, /currently in use/);

  const quotaResult = await core.queryQuota();
  assert.equal(quotaResult.kind, "unsupported");
  assert.equal(quotaResult.modeName, "cliproxyapi");
});

test("useAccount restores a shared relay route", (t) => {
  const codexHome = createTempCodexHome();
  process.env.CODEX_HOME = codexHome;
  core.setNamedAuthDir(codexHome);
  t.after(() => fs.rmSync(codexHome, { recursive: true, force: true }));
...
test("corrupt shared route state does not crash status helpers", async (t) => {
  ...
  core.setNamedAuthDir(codexHome);
  t.after(() => fs.rmSync(codexHome, { recursive: true, force: true }));
  writeJson(path.join(codexHome, "auth.json"), { OPENAI_API_KEY: "sk-relay" });
  core.writeProviderProfile({
    kind: "provider",
    name: "cliproxyapi",
    auth: { OPENAI_API_KEY: "sk-relay" },
    config: { name: "cliproxyapi", base_url: "https://proxy.example/v1", wire_api: "responses" },
  });
  writeJson(path.join(codexHome, "switchbridge-shared-history.json"), {
    version: 1,
  ...
  assert.doesNotThrow(() => core.getCurrentSelection());
  assert.doesNotThrow(() => core.listAccounts());
  await assert.doesNotReject(() => core.queryQuota());

  const deleteResult = core.deleteProviderProfile("cliproxyapi");
  assert.equal(deleteResult.success, false);
  assert.match(deleteResult.message, /shared-history route state/i);
...
test("a delayed local-account refresh cannot overwrite a committed provider switch", async () => {
  const codexHome = createTempCodexHome();
  process.env.CODEX_HOME = codexHome;

  const accountAuth = makeAccountAuth("acct-work", "rt-current", "access-current");
  writeJson(path.join(codexHome, "auth_work.json"), accountAuth);
  writeJson(path.join(codexHome, "auth.json"), accountAuth);
  ...
  const refreshedIdToken = makeAccountAuth("acct-work", "unused", "unused").tokens.id_token;
  const delayedRefreshRequest = (options, handler) => {
    const response = new EventEmitter();
  ...
  await withMockedHttpsRequest(delayedRefreshRequest, async () => {
    const refreshPromise = core.refreshAccount("work");
    ...
    core.activateProviderProfile(
      ...
      { shareHistoryAcrossProviders: true, source: "account:work", target: "provider:proxy" },
    );
    releaseResponse();
...
test("writeAuthFile strips legacy refresh_token_expires_at fields while preserving auth freshness", () => {
  ...
  core.writeAuthFile(authPath, {
    ...
    codex_switchbridge_auth_updated_at: authUpdatedAt,
    refresh_token_expires_at: "2099-01-01T00:00:00.000Z",
    ignored_field: "ignored",
  });

  const saved = JSON.parse(fs.readFileSync(authPath, "utf-8"));
  assert.equal(saved.codex_switchbridge_auth_updated_at, authUpdatedAt);

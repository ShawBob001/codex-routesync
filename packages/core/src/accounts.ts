...
const LOCK_LOG_PREFIX = "[codex-switchbridge:lock]";
...
export async function queryQuota(name?: string, options: AccountQuotaQueryOptions = {}): Promise<QuotaQueryResult> {
  const perf = createDiagnosticPerformanceTimer(
    "[codex-switchbridge:core:accounts]",
    "queryQuota",
    {
...
export async function refreshAccount(name?: string, options: SavedAccountOperationOptions = {}): Promise<{
  ...
  try {
    return await withAccountLock(initialTarget.auth, "refreshAccount", async () => {
      ...
      const shouldWriteCurrentAuth = shouldSyncCurrentAuth || authPath === getCodexAuthPath();
      const currentAuthWriteGuard = shouldWriteCurrentAuth
        ? captureCurrentAuthWriteGuard(auth)
        : null;
      ...
      try {
        ...
        } else if (authPath !== getCodexAuthPath()) {
          writeSavedAuthFile(authPath, updated);
        }
        if (currentAuthWriteGuard) {
          writeCurrentAuthIfUnchanged(currentAuthWriteGuard, updated);
        }

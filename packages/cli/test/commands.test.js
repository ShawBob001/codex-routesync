...
const CLI_MANIFEST = require("../package.json");

// ─── Helpers ─────────────────────────────────────────────────

function tmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "csb-cli-test-"));
}
...
function cli(args, home, opts = {}) {
  ...
  delete env.CODEX_SWITCHBRIDGE_AUTH_DIR;
...
function cliDiagnosticLogPath(home) {
  return path.join(home, "logs", "codex-switchbridge-cli.log");
}
...
test("--version prints version number", () => {
  ...
  assert.equal(r.stdout.trim(), CLI_MANIFEST.version);
});
...
test("add: uses browser login by default", () => {
  const home = tmpHome();
  const binDir = fs.mkdtempSync(path.join(os.tmpdir(), "csb-cli-bin-"));
...
test("password: CODEX_SWITCHBRIDGE_PASSWORD env var decrypts encrypted account", () => {
  const home = tmpHome();
  ...
  const r = cli("list", home, { env: { CODEX_SWITCHBRIDGE_PASSWORD: "env-pass" } });
  assert.equal(r.code, 0);
...
test("password: --password takes precedence over env var", () => {
  ...
  const r = cli('--password "cli-pass" list', home, {
    env: { CODEX_SWITCHBRIDGE_PASSWORD: "wrong-pass" },
  });
  assert.equal(r.code, 0);
...
test("password: --help shows password option", () => {
  ...
  assert.ok(r.stdout.includes("CODEX_SWITCHBRIDGE_PASSWORD"));

  ...
  if (hasLocked || hasLockedProviders) {
    console.log(
      chalk.redBright.bold(
        "\n  Some saved entries are locked. Use --password <password> or set CODEX_SWITCHBRIDGE_PASSWORD to decrypt them."
      )
    );
  }
...
export async function cmdRefresh(name?: string): Promise<void> {
  ...
  if (!result.success) {
    ...
    if (!result.unsupported) {
      console.log(chalk.yellow("Try logging in again: codex-switchbridge add <name>"));
    }
    return;
  }

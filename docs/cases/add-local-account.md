...
| local account 读取 auth 文件兼容性 | 本地账号目录同时存在未加密 `AuthFile` JSON 与加密 `saved_auth` envelope 文件 | 账号列表、切换账号、配额查询都能正常读取对应 local account；未加密文件继续可读，加密文件在已加载本地存储密码时可读。 |
...
| CLI 新增账号 | 用户当前真实 `auth.json` 为 `google1`，运行 `codex-switchbridge add bob1990` 并完成网页登录 | CLI 将 `bob1990` 保存为 `auth_bob1990.json`，真实 `auth.json` 仍为 `google1`，当前账号不被隐式切换。 |

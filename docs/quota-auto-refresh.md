...
| Area | Owner | Notes |
| --- | --- | --- |
| Background quota timer | `RefreshCoordinator` | Extension-level single timer, keyed by `codex-switchbridge.quotaRefreshInterval`. |
...
| Token maintenance boundary | `RefreshCoordinator` | timer 周期可在 quota 前按 `codex-switchbridge.tokenAutoUpdate` 单独执行 token 检查；详细规则见 `docs/token-auto-refresh.md`。 |

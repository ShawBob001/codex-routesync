...
SwitchBridge updates files on disk immediately. A running Codex extension may still cache authentication or history state in memory. The default `statusBar` reload policy shows one non-blocking action when a reload is useful. It does not display repeated notification popups.

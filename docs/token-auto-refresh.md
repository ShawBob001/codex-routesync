...
| Auto-refresh setting | `codex-switchbridge.tokenAutoUpdate` | Defaults to `true`; when disabled, the timer skips token maintenance and still runs the independent quota step. |
...
flowchart LR
  ...
  B --> E{tokenAutoUpdate enabled?}
  E -->|no| G[Continue with independent quota refresh]
  E -->|yes| C{Token expires within 120h?}
...
| Setting | `codex-switchbridge.tokenAutoUpdate` defaults to `true`; setting it to `false` disables automatic token refresh only. |
...
| Quota token rejected | If the quota API returns `401/403`, including `error.code = token_invalidated`, the quota result is mapped to `quota_token_rejected` and surfaces `Quota API rejected current token` instead of the misleading `Relogin required`. |

...
| Area | Storage | Notes |
| --- | --- | --- |
| Cloud account index | VS Code `globalState` synced key `codex-switchbridge.syncedCloudState.v1` | Stores account names plus shared cloud metadata; account payloads are not stored in this aggregate key. |
| Cloud accounts | Per-account VS Code `globalState` synced keys `codex-switchbridge.syncedCloudAccount.v1.{name}` | Payload stays encrypted with the saved-auth passphrase; updating one account does not rewrite sibling accounts. |
| Cloud providers | Per-provider VS Code `globalState` synced keys `codex-switchbridge.syncedCloudProvider.v1.{name}` | Uses the same encrypted envelope format as accounts, plus sync revision metadata and provider audit metadata (`lastWriterAction`). |
...
| Account auth freshness | `codex_switchbridge_auth_updated_at` inside account auth | Internal timestamp used to reject stale auth writes when multiple machines sync the same cloud account. |
...
| Rule | Behavior |
| --- | --- |
| Cloud account writes | Persisted account auth carries `codex_switchbridge_auth_updated_at`. |
...
| Legacy cleanup succeeds | Remove the old `codex-switchbridge.syncedStorage` setting. |
...
| Envelope format must stay unchanged | `@codex-switchbridge/core` remains the canonical serializer/deserializer. |

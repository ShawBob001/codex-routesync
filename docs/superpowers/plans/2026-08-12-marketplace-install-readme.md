...
**Architecture:** Keep installation instructions in the two existing README files and use a brand-only Marketplace search URL. Treat GitHub Releases as the offline fallback, and update the repository homepage only after the documentation PR is merged.
...
const marketplace = "https://marketplace.visualstudio.com/search?sortBy=Relevance&term=Codex%20SwitchBridge&target=VSCode";
...
assert.match(root, /VS%20Code%20Marketplace-install/i);
NODE
```

Expected: FAIL because both README files lack the Marketplace search URL and still name the 0.3.0 VSIX.
...
```markdown
[Find Codex SwitchBridge in the Visual Studio Marketplace](https://marketplace.visualstudio.com/search?sortBy=Relevance&term=Codex%20SwitchBridge&target=VSCode), or open **Extensions** in VS Code and search for `Codex SwitchBridge`.

For offline installation, download the latest `.vsix` from [GitHub Releases](https://github.com/ShawBob001/codex-switchbridge/releases), then run **Extensions: Install from VSIX...** or:
...
Use the same Marketplace search link, brand search term, offline fallback, and version-neutral shell command from Step 2. Keep the existing warning about disabling conflicting switch extensions immediately after installation instructions.

- [ ] **Step 4: Humanize and consistency-check the text**

Read both edited sections aloud as technical instructions. Remove promotional filler, repeated phrasing, em/en dashes, and claims not supported by the Marketplace page. Keep the product name, URL, and commands exact.
...
curl -fsSL -A 'Mozilla/5.0' -o /dev/null -w '%{http_code}\n' \
  'https://github.com/ShawBob001/codex-switchbridge/releases'
```
...
```bash
gh repo edit ShawBob001/codex-switchbridge \
  --homepage 'https://marketplace.visualstudio.com/search?sortBy=Relevance&term=Codex%20SwitchBridge&target=VSCode'
```

- [ ] **Step 3: Verify the merged repository state**

```bash
gh repo view ShawBob001/codex-switchbridge \
  --json defaultBranchRef,description,homepageUrl,url
```

Expected: `main` is the default branch, the existing product description is unchanged, and `homepageUrl` is the Marketplace search URL.

...
## VS Code 0.8.0 release procedure

GitHub Actions verifies pushes and pull requests but does not publish releases. Run this procedure from a clean, current `main` checkout after CI passes. The public extension identity is `ShawBob001.codex-switchbridge-vscode`, and its direct Marketplace page is [Codex SwitchBridge for VS Code](https://marketplace.visualstudio.com/items?itemName=ShawBob001.codex-switchbridge-vscode).

Fetch remote tags, confirm that 0.8.0 is unused, then package and verify one VSIX. Copy it to a temporary release directory before calculating its checksum; all uploads below use that immutable copy.
...
gh auth status
gh repo view ShawBob001/codex-switchbridge --json nameWithOwner >/dev/null
if gh release view v0.8.0 --repo ShawBob001/codex-switchbridge >/dev/null 2>&1; then
  echo "v0.8.0 already exists" >&2; exit 1
fi
npm ci
npm run verify
...
RELEASE_VSIX="$RELEASE_DIR/codex-switchbridge-vscode-0.8.0.vsix"
RELEASE_CHECKSUM="$RELEASE_DIR/codex-switchbridge-v0.8.0-SHA256SUMS"
cp packages/vscode/codex-switchbridge-vscode-0.8.0.vsix "$RELEASE_VSIX"
unzip -t "$RELEASE_VSIX"
unzip -p "$RELEASE_VSIX" extension/package.json > "$RELEASE_DIR/extension-package.json"
node -e 'const m=require(process.argv[1]); if (m.publisher!=="ShawBob001" || m.name!=="codex-switchbridge-vscode" || m.version!=="0.8.0") process.exit(1)' "$RELEASE_DIR/extension-package.json"
! unzip -Z1 "$RELEASE_VSIX" | rg -q '(^|/)(\.env[^/]*|\.npmrc|test|src)(/|$)'
(cd "$RELEASE_DIR" && sha256sum codex-switchbridge-vscode-0.8.0.vsix > codex-switchbridge-v0.8.0-SHA256SUMS)
(cd "$RELEASE_DIR" && sha256sum --check codex-switchbridge-v0.8.0-SHA256SUMS)
```
...
```bash
git tag -a v0.8.0 -m "Codex SwitchBridge 0.8.0"
git push origin v0.8.0
sed -n '/^## 0\.8\.0 /,/^## 0\.7\.0 /p' packages/vscode/CHANGELOG.md | sed '$d' > "$RELEASE_DIR/release-notes.md"
gh release create v0.8.0 \
  "$RELEASE_VSIX" \
  "$RELEASE_CHECKSUM" \
  --draft \
  --verify-tag \
  --title "Codex SwitchBridge 0.8.0" \
  --notes-file "$RELEASE_DIR/release-notes.md"
VERIFY_DIR="$(mktemp -d)"
gh release download v0.8.0 \
  --dir "$VERIFY_DIR" \
  --pattern 'codex-switchbridge-vscode-0.8.0.vsix' \
  --pattern 'codex-switchbridge-v0.8.0-SHA256SUMS'
(cd "$VERIFY_DIR" && sha256sum --check codex-switchbridge-v0.8.0-SHA256SUMS)
test "$(gh release view v0.8.0 --json assets --jq '.assets | map(.name) | sort | join(" ")')" = "codex-switchbridge-v0.8.0-SHA256SUMS codex-switchbridge-vscode-0.8.0.vsix"
```
...
```bash
npx --yes @vscode/vsce@3.9.2 show ShawBob001.codex-switchbridge-vscode --json
code --install-extension ShawBob001.codex-switchbridge-vscode --force
code --list-extensions --show-versions | rg '^ShawBob001\.codex-switchbridge-vscode@0\.8\.0$'
```

Publish the GitHub draft only after the Marketplace page reports 0.8.0 and the downloaded GitHub assets pass their checksum.

```bash
gh release edit v0.8.0 --draft=false
gh release view v0.8.0 --json url,tagName,isDraft,assets
```

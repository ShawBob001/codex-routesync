...
## VS Code 0.8.1 release procedure
...
Fetch remote tags, confirm that 0.8.1 is unused, then package and verify one VSIX. Copy it to a temporary release directory before calculating its checksum; all uploads below use that immutable copy.
...
test -z "$(git tag --list v0.8.1)"
gh auth status
gh repo view ShawBob001/codex-routesync --json nameWithOwner >/dev/null
if gh release view v0.8.1 --repo ShawBob001/codex-routesync >/dev/null 2>&1; then
  echo "v0.8.1 already exists" >&2; exit 1
fi
npm ci
npm run verify
...
RELEASE_VSIX="$RELEASE_DIR/codex-routesync-0.8.1.vsix"
RELEASE_CHECKSUM="$RELEASE_DIR/codex-routesync-v0.8.1-SHA256SUMS"
cp packages/vscode/codex-routesync-0.8.1.vsix "$RELEASE_VSIX"
unzip -t "$RELEASE_VSIX"
unzip -p "$RELEASE_VSIX" extension/package.json > "$RELEASE_DIR/extension-package.json"
node -e 'const m=require(process.argv[1]); if (m.publisher!=="ShawBob001" || m.name!=="codex-routesync" || m.version!=="0.8.1") process.exit(1)' "$RELEASE_DIR/extension-package.json"
! unzip -Z1 "$RELEASE_VSIX" | rg -q '(^|/)(\.env[^/]*|\.npmrc|test|src)(/|$)'
(cd "$RELEASE_DIR" && sha256sum codex-routesync-0.8.1.vsix > codex-routesync-v0.8.1-SHA256SUMS)
(cd "$RELEASE_DIR" && sha256sum --check codex-routesync-v0.8.1-SHA256SUMS)
```
...
```bash
git tag -a v0.8.1 -m "Codex RouteSync 0.8.1"
git push origin v0.8.1
sed -n '/^## 0\.8\.1 /,/^## 0\.8\.0 /p' packages/vscode/CHANGELOG.md | sed '$d' > "$RELEASE_DIR/release-notes.md"
gh release create v0.8.1 \
  "$RELEASE_VSIX" \
  "$RELEASE_CHECKSUM" \
  --draft \
  --verify-tag \
  --title "Codex RouteSync 0.8.1" \
  --notes-file "$RELEASE_DIR/release-notes.md"
VERIFY_DIR="$(mktemp -d)"
gh release download v0.8.1 \
  --dir "$VERIFY_DIR" \
  --pattern 'codex-routesync-0.8.1.vsix' \
  --pattern 'codex-routesync-v0.8.1-SHA256SUMS'
(cd "$VERIFY_DIR" && sha256sum --check codex-routesync-v0.8.1-SHA256SUMS)
test "$(gh release view v0.8.1 --json assets --jq '.assets | map(.name) | sort | join(" ")')" = "codex-routesync-0.8.1.vsix codex-routesync-v0.8.1-SHA256SUMS"
```
...
code --list-extensions --show-versions | rg '^ShawBob001\.codex-routesync@0\.8\.1$'
```

Publish the GitHub draft only after the Marketplace page reports 0.8.1 and the downloaded GitHub assets pass their checksum.

```bash
gh release edit v0.8.1 --draft=false
gh release view v0.8.1 --json url,tagName,isDraft,assets
```
